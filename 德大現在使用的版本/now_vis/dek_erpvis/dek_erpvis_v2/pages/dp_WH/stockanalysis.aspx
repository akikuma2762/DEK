...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <script>
        ...
        //產生表格的HTML碼
        create_tablecode('stockanalysis', '逾期數量列表', 'datatable-buttons', '<%=th.ToString() %>', '<%=tr.ToString() %>');
        //產生相對應的JScode
        set_Table('#datatable-buttons');
        //防止頁籤跑版
        loadpage('stockanalysis=stockanalysis_cust', '#datatable-buttons');

        //產生表格的HTML碼
        create_tablecode('money', '庫存金額列表', 'Money_DataTable', '<%=th_money.ToString() %>', '<%=tr_money.ToString() %>');
        //產生相對應的JScode
        set_Table('#Money_DataTable');
        //防止頁籤跑版
        loadpage('', '');
