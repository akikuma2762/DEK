...
namespace dek_erpvis_v2.page_webservice
{
    public partial class Shipment : System.Web.UI.Page
    {
        ...
        private void set_col_value_cust(string count)
        {
            ...
            if (dt.Rows.Count >= DataTableUtils.toInt(count))
                max = DataTableUtils.toInt(count);
            else if (dt.Rows.Count < DataTableUtils.toInt(count))
                max = dt.Rows.Count;

            for (int i = 0; i < max; i++)                                 //3.利用迴圈組合長條圖資料(ru)
                col_data_Points_cust += "{ y: " + dt.Rows[i]["數量"].ToString() + ", label:'" + dt.Rows[i]["客戶簡稱"].ToString() + "',indexLabel:'" + dt.Rows[i]["數量"].ToString() + "台'  },";
            dt.Dispose();
            dt.Clear();
        }
