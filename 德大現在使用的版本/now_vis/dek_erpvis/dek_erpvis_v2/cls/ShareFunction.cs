...
namespace dek_erpvis_v2.cls
{
    public class ShareFunction
    {
        ...
        //設定啟動時間 暫停時間 完成時間
        public bool change_status(string acc, string Link, DataTable dt, string tablename, string condition, string status, string Report, string dispatch, string percent = "")
        {
...
            return t;
        ...
        public string month_WorkDay(string date_str, string date_end)
        {
            ...
            int day = int.Parse(((endDay - startDay).Days + 1).ToString());
            string sqlcmd = $"SELECT count(IsDay)月假日 FROM WorkTime_Holiday where PK_Holiday between {date_str} and {date_end}";
            GlobalVar.UseDB_setConnString(myclass.GetConnByDekdekVisAssm);
    ...
    //20221223 生產推移圖用
    public class ProductionProgressChart {
        ...
        //取得本月應做資料
        public DataTable Get_ProductionDataTable(string date_str, string date_end,string condition,string factory)
        {
            DataTable dt = new DataTable();
            switch (factory)
            {
                case "sowon":
                    dt = Get_AssmDataTable(date_str, date_end, condition);
                    dt = myclass.Add_LINE_GROUP(dt).ToTable();
                    break;
                case "iTec":
                    dt = Get_HorDataTable(date_str, date_end, condition);
                    dt = HorTransProductionLineNumber(dt);
                    dt = myclass.Add_LINE_GROUP(dt).ToTable();
                    break;
            }
            return dt;
        }
        public DataTable Get_AssmDataTable(string date_str, string date_end,string condition)
        {
        ...
        public DataTable Get_HorDataTable(string date_str, string date_end,string condition)
        {
            DataTable dt_month = new DataTable();
        ...
        //---------------------------產能-------------------------//
        public DataTable Get_CapacityDatable(string factory)
        {
            DataTable dt = new DataTable();
            switch (factory)
            {
        ...
        public DataTable Get_AssmCapacity()
        {
            DataTable dt = new DataTable();
            GlobalVar.UseDB_setConnString(myclass.GetConnByDekdekVisAssm);
            dt = DataTableUtils.GetDataTable("select 工作站編號 AS 產線代號,工作站名稱,目標件數 AS 日產能 FROM 工作站型態資料表 WHERE 工作站是否使用中='1'");
            dt = myclass.Add_LINE_GROUP(dt).ToTable();
            return dt;

        }
        public DataTable Get_HorCapacity()
        {
            DataTable dt = new DataTable();
            GlobalVar.UseDB_setConnString(myclass.GetConnByDekdekVisAssmHor);
            dt = DataTableUtils.GetDataTable("select (CASE when 工作站編號='1' then '100' when 工作站編號='2' then'110' else 工作站編號 End) AS 產線代號,工作站名稱,目標件數 AS 月產能 FROM 工作站型態資料表 WHERE 工作站是否使用中='1'");
            dt = HorTransProductionLineNumber(dt);
            dt = myclass.Add_LINE_GROUP(dt).ToTable();
            return dt;
        ...
        public DataTable HorTransProductionLineNumber(DataTable dt)
        {
            DataTable dt_clone = dt.Clone();
            dt_clone.Columns["產線代號"].ReadOnly = false;
            dt_clone.Columns["產線代號"].DataType = typeof(string);
            dt_clone.Columns["產線代號"].MaxLength = 67;
        ...
        //單獨取的大圓盤資料
        public DataTable get_大圓盤(string date_str, string date_end)
        {
            DataTable dt_month = new DataTable();
            string sqlcmd = "";
            //立式廠 && 大圓盤資訊
        ...
        public string ProductionProgressTable_CallBack(List<ProductionProgress> productionProgresses, DataRow row, string field_name)
        {
            ...
            foreach (ProductionProgress productionProgress in productionProgresses)
            {
                ...
                dt_Staff = productionProgress.StaffDataTable;
                error_unsolved = productionProgress.ErrorUnsolvedDataTable;
                dt_capacity = productionProgress.CapacitydDataTable;
                acc = productionProgress.Account;
                d_end = productionProgress.Date_End;
                d_start = productionProgress.Date_Start;
                condition = productionProgress.Condition;
            }
            string sqlcmd = "";
            int _預定生產 = 0;
            int _預計生產量 = 0;
                ...
                switch (row_name)
                {
                    case "月產能":
                        if (factoryFloor == "sowon")
                        {
                            ...
                            int workDay = int.Parse(sfun.month_WorkDay(d_start, d_end));
                    ...
                    case "本期計畫生產":
                        //預計生產
                        sqlcmd = ExpectedProduction_CMD(d_end, today, d_start, true);
                    ...
                    case "昨日應有進度":
                        ...
                        if (day.Contains("26"))
                        {
                            ...
                            switch (factoryFloor)
                            {
                                case "sowon":
                                    ...
                                    break;
                                case "iTec":
                                    DataTable last_month_Hor = Get_ProductionDataTable(startDay, endDay, condition, factoryFloor);
                        ...
                        {
                            if (int.Parse(d_end) < int.Parse(today)) 
                            {
                                yesterday = DateTime.ParseExact(d_end, "yyyyMMdd", null, System.Globalization.DateTimeStyles.AllowWhiteSpaces).AddDays(-1).ToString("yyyyMMdd");
                                sqlcmd = ExpectedProduction_CMD(yesterday, today, d_start, true);
                            }
                            else 
                            {
                                sqlcmd = ExpectedProduction_CMD(yesterday, today, d_start, true);
                        ...
                        sqlcmd += $" AND 產線群組='{LineGroup}'";
                        rows = dt_factory.Select(sqlcmd);
                        //DataTable dtsss = dt_factory.Clone();
                        //for (int i = 0; i < rows.Length; i++)
                    ...
                    case "昨日實際進度":
                        ...
                        else
                        {
                            ...
                            {
                                Dictionary<string, string> dc = new Dictionary<string, string>();
                                dc = sfun.monthInterval(d_end, acc);
                                string endDay  = dc["endDay"].ToString();
                                yesterday = DateTime.ParseExact(d_end, "yyyyMMdd", null, System.Globalization.DateTimeStyles.AllowWhiteSpaces).AddDays(-1).ToString("yyyyMMdd");
                                sqlcmd = ProductionFullCountForYesterday_CMD(d_start, endDay, yesterday);
                            }
                            else
                            {
                                Dictionary<string, string> dc = new Dictionary<string, string>();
        ...
        public string ExpectedProduction_CMD(string expected_fn_day, string today, string date_str, bool first_day)
        {
            ...
            if (DataTableUtils.toInt(today) >= DataTableUtils.toInt(date_str))
                ...
                //2.顯示已在上期提早完成的數量
                sqlcmd = first_day ? $"((預計完工日 <= '{expected_fn_day}' and (substring(實際完成時間, 1, 8)>='{date_str}' OR substring(實際完成時間, 1, 8)<='{date_str}' OR 實際完成時間 IS NULL  OR 實際完成時間 =''))  OR 預計完工日='開發機' ) " : $"預計完工日 = '{expected_fn_day}' and (substring(實際完成時間, 1, 8)>='{date_str}' OR substring(實際完成時間, 1, 8)<='{date_str}' OR 實際完成時間 IS NULL  OR 實際完成時間 ='')";
            else
        ...
        public string ProductionFullCountForYesterday_CMD(string start, string end, string yesterday)
        {
            string sqlcmd = "";
            //不顯示已在上期完成的數量
            //sqlcmd = $"(預計完工日<={end} and 實際完成時間 >='{start}' and 實際完成時間 <='{end}') OR (預計完工日<={end} and 實際完成時間<={yesterday}) and 狀態 = '完成'";
            //顯示已在上期完成的數量
